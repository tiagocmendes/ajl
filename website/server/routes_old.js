...
router.get('/knockouts', async (req, res) => {
	try {
		...
		for (const group of groupsClassification) {
			const teamsThatPlayedAllMatches = group.teams.filter(
				team =>
					team.victories + team.draws + team.losses === group.teams.length - 1
			);
			const allMatchesPlayed =
				teamsThatPlayedAllMatches.length === group.teams.length;
		...
		for (let i = 0; i < semi.length; i++) {
			...
			const looser =
				winner !== '' && semi[i].firstTeam.name !== winner
					? semi[i].firstTeam.name
					: semi[i].secondTeam.name;
			if (winner !== '' && looser) {
				if (i === 0) {
